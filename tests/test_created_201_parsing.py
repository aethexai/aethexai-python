"""Regression tests: HTTP 201 Created responses must be parsed.

The Aethex API returns ``201 Created`` from resource-creation POSTs as well as
``200 OK``. The generated ``_parse_response`` functions only branched on ``200``
and fell through to ``return None`` on ``201``; ``_call`` returns
``response.parsed`` for any 2xx, so callers could silently lose the created
resource. The SDK now parses ``201`` the same way it parses ``200``.
...
def test_create_agent_parses_201_body(client: AethexAI) -> None:
    ...
    upgraded from raw dict to ``AgentResponse``; access via attribute

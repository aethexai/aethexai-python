...
def test_list_voices_forwards_query_params(client: AethexAI) -> None:
    ...
    client.list_voices(language="french", tag="warm", country="NG", limit=10, offset=5)
...
# preview_voice and stream_audio return ``audio/wav`` bytes even
...
def test_cancel_transcription_job_returns_typed_model(client: AethexAI) -> None:
    """DELETE /transcribe/{job_id} parses to ``CancelTranscriptionJobResponse``.
...
# ─── usage ─────────────────────────────────────────────────────────────
#
# (Billing methods moved to ``DeveloperClient`` —

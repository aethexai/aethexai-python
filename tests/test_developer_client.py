"""Tests for ``DeveloperClient`` / ``AsyncDeveloperClient``.
...
* No-leak: access and refresh tokens never appear in repr / vars /
  exception messages (no-key-leak regression)
"""
...
def test_select_plan_default_body_no_unset_crash() -> None:
    # Regression: the natural ``select_plan('pro')`` call must
...
async def test_select_plan_default_body_no_unset_crash_async() -> None:
    # Async counterpart of the regression.
    route = respx.post(f"{BASE_URL}/api/v1/billing/plans/pro/select").respond(
        200,
        json={"plan_slug": "pro", "status": "active"},
    )
...
# ─── No-leak invariants (no-key-leak regression) ───────────────────
...
def test_developer_client_does_not_store_access_token_on_instance() -> None:
    ...
    try:
        ...
        # object, not a string, and its repr-suppression is the
...
def test_aethexai_no_longer_exposes_billing_methods() -> None:
    ...
    for name in (
        ...
        assert not hasattr(AethexAI, name), (
            f"AethexAI.{name} must have been removed — "

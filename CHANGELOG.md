...
### Fixed
- `RecordingResponse.storage_path` is now optional (`UNSET` default) so the SDK tolerates the backend dropping this internal field (expand step for the storage_path removal; see aethex#1007). Previously the generated `from_dict` did `d.pop("storage_path")` with no default, raising `KeyError` for any client deserializing a recording once the backend stops sending the field. The field is still emitted today — this change purely widens tolerance ahead of the backend removal. A durable post-codegen patch in `scripts/sync_from_prod.py` re-applies the optional declaration after any future regen so a `sync_from_prod.py --apply` run cannot silently revert the fix. (AET-1599)
...
- `create_agent`, `update_agent`, and `duplicate_agent` now return a typed `AgentResponse` instead of a raw `dict`. The `openapi.json` committed here re-aligns with the deployed backend: `create_agent` and `update_agent` already declare `response_model=AgentResponse` on the backend; `duplicate_agent` will match once its backend PR ships. A durable post-codegen patch (`_apply_typed_agent_response_patch` in `scripts/sync_from_prod.py`) guarantees `AgentResponse.from_dict()` parsing for all three endpoints — including `update_agent` (PATCH 200), which was not covered by the prior AET-1580 patch — across every future regeneration, even if `openapi.json` temporarily reverts to `{}` for any of them. (AET-1597)

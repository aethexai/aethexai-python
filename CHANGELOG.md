...
- `RecordingResponse.storage_path` is now optional. The generated `from_dict` previously required the field and raised `KeyError` when it was absent; the field is now tolerated whether the response includes it or not, so deserializing a recording is forward-compatible with the field being dropped.
